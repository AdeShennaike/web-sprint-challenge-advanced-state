...
export function fetchQuiz() {
  ...
  export function postQuiz(payload, stateMess) {
    return function (dispatch) {
      axios.post('http://localhost:9000/api/quiz/new', {question_text: payload.newQuestion, true_answer_text: payload.newTrueAnswer, false_answer_text: payload.newFalseAnswer})
      .then(res =>{
      dispatch({type: types.SET_INFO_MESSAGE, payload: `Congrats: "${stateMess}" is a great question!`})
